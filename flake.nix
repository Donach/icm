      ...
      packages = forAll (
        ...
        in
        {
          icm = pkgs.rustPlatform.buildRustPackage {
            ...
            # libstdc++, which is newer than the Rust toolchain's own libstdc++
            # that lands first on the binary's RPATH. Without help the binary
            # aborts at startup with `GLIBCXX_3.4.xx not found (required by
            # libonnxruntime.so)`. Put the matching libstdc++ (and onnxruntime)
            # on the loader path so the binary runs standalone, outside a shell.
            postInstall = ''
