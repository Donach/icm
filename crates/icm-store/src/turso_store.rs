//! Turso/libSQL storage backend for ICM.
//!
//! This is a port of the SQLite backend (`store.rs`) that routes the same
//! SQL through the `dbcompat` shim — a synchronous rusqlite-shaped facade
//! over the async `libsql` client. The shim is aliased as `rusqlite` so
//! the SQL logic below is byte-identical to `store.rs`; only the connection
//! open path and PRAGMA handling differ.
//!
//! Active when `ICM_DB_BACKEND=turso` (or `TURSO_DATABASE_URL` is set).
...
use lru::LruCache;
// Turso/libSQL backend: route SQL through the libSQL-backed dbcompat shim,
// aliased as `rusqlite` so the rest of this file is byte-identical to upstream.
...
use crate::turso_schema::init_db_with_dims;
...
// sqlite-vec is registered inside dbcompat's Connection::open
// (after libsql's threading init), so this is a no-op for the Turso backend.
fn ensure_sqlite_vec() {}

// open_readonly_immutable not applicable to Turso remote backend
...
impl TursoStore {
    ...
    pub fn open_readonly(path: &Path) -> IcmResult<Self> {
        // Remote Turso databases do not support the read-only/immutable
        // open path (no SQLITE_OPEN_READ_ONLY over the network). For local
        // libSQL files, fall back to a writable connection opened read-only
        // in spirit by setting `readonly = true`.
        ensure_sqlite_vec();
        let url = std::env::var("TURSO_DATABASE_URL")
            .or_else(|_| std::env::var("LIBSQL_URL"))
            .ok()
            .filter(|s| !s.trim().is_empty());
        let conn = if url.is_some() {
    ...
    pub fn read_stored_embedding_dims(path: &Path) -> IcmResult<Option<usize>> {
        // For Turso/remote DBs, we can still query metadata; the path is
        // used for local files only.
        let url = std::env::var("TURSO_DATABASE_URL")
            .or_else(|_| std::env::var("LIBSQL_URL"))
            .ok()
            .filter(|s| !s.trim().is_empty());
    ...
    /// Pick the storage backend from the environment, else a local file.
    ///
    /// * `TURSO_DATABASE_URL` (or `LIBSQL_URL`) + `ICM_TURSO_REPLICA` truthy →
    ///   local embedded replica at `path` syncing to the remote primary.
    /// * URL set otherwise → remote libSQL/Turso server (`sqld`/`tursodb`);
    ///   every ICM process shares it, serialising concurrent writes server-side.
    /// * no URL → local libSQL file at `path` (same SQL dialect as SQLite).
    ///
    /// Auth token: `TURSO_AUTH_TOKEN` (or `LIBSQL_AUTH_TOKEN`); empty for an
    /// unauthenticated local `sqld`.
    fn open_connection(path: &Path) -> IcmResult<Connection> {
        let url = std::env::var("TURSO_DATABASE_URL")
            .or_else(|_| std::env::var("LIBSQL_URL"))
            .ok()
            .filter(|s| !s.trim().is_empty());
    ...
    fn apply_pragmas(conn: &Connection) -> IcmResult<()> {
        if conn.is_remote() {
            // Remote server manages journaling/locking; best-effort FK enforcement.
...
impl MemoirStore for TursoStore {
    // --- Memoir CRUD ---
...
impl FeedbackStore for TursoStore {
...
impl FactsStore for TursoStore {
...
impl TranscriptStore for TursoStore {
    fn create_session(
        &self,
        agent: &str,
        project: Option<&str>,
...
mod tests {
    ...
    fn test_store() -> TursoStore {
        TursoStore::in_memory().unwrap()
    }
    ...
    fn seed_writable_db(path: &Path) -> Memory {
        let store = TursoStore::new(path).unwrap();
    ...
    fn open_readonly_errors_on_missing_file() {
        ...
        match TursoStore::open_readonly(&path) {
    ...
    fn open_readonly_can_read_existing_db() {
        ...
        let ro = TursoStore::open_readonly(&path).unwrap();
    ...
    fn read_only_recall_path_skips_access_bookkeeping() {
        ...
        let rw = TursoStore::new(&path).unwrap();
    ...
    fn read_stored_dims_returns_none_for_missing_file() {
        ...
        assert_eq!(
            TursoStore::read_stored_embedding_dims(&path).unwrap(),
            None
        );
    }
    ...
    fn read_stored_dims_returns_stored_value_for_populated_db() {
        ...
        let store = TursoStore::with_dims(&path, 1024).unwrap();
        drop(store);
    ...
    fn opening_at_stored_dims_preserves_vectors() {
        ...
        let stored = TursoStore::read_stored_embedding_dims(&path)
            .unwrap()
            .expect("stored dim must be readable on populated DB");
        assert_eq!(stored, 1024);

        let store = TursoStore::with_dims(&path, stored).unwrap();
