//! Schema initialisation for the Turso/libSQL backend.
//!
//! Byte-identical to `schema.rs` except that `rusqlite` is aliased to the
//! `dbcompat` shim so the same SQL runs over libSQL connections.
